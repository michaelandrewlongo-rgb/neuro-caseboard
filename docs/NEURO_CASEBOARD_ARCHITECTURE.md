...
| **P4 — Output Generation & Rendering** | `neuro_caseboard/{render_md,render_pdf,briefing_pdf,operative_briefing_pdf,caseboard_pdf,fpdf_base,exec_navy,board_view,captions,briefing_figures}.py`, `figures_gen/`, `assets/` | Turns in-memory models into Markdown, PDF, and generated schematic figures. |
| **P5 — Interfaces, Surfaces & Deployment** | `neuro_caseboard/cli.py`, `api/`, `web/`, `scripts/`, `ci/`, `.github/`, `Dockerfile`, `tests/` | CLI / FastAPI / React surfaces over the one engine, plus build/deploy/test plumbing. |
...
- **`entailment.py` — the citation entailment gate.** The production default is `NLIVerifier`, a lazily-imported `sentence_transformers.CrossEncoder` semantic entailment model (`tasksource/deberta-base-long-nli`, MNLI-aware via `id2label`; markdown stripped from both sides; flag when `P(entailment) < 0.3`, threshold chosen for fabrication recall via an out-of-sample two-lab judge panel — see `evaluation/scripts/judge_verifier.py` and `evaluation/RESULTS.md`). The verifier instance is cached per `(model, threshold)`; a failed load is cached too and degrades once to `LexicalVerifier`, the deterministic dependency-free fallback: token-overlap recall over the whole premise (`threshold=0.18`) AND precision over the best-matching premise *sentence* (`min_precision=0.2`, `min_premise_tokens=5`). Env `CASEBOARD_NLI_MODEL` overrides the model (`lexical`/empty selects the fallback); `CASEBOARD_NLI_THRESHOLD` overrides the flag threshold. `should_cite(premise, hypothesis, verifier)` keeps a citation unless a *judgeable* premise is positively rejected (abstain→keep below `min_premise_tokens`); `unsupported_entities` flags cross-source "bleed" via a medical-suffix regex.
...
P4 turns two P2 models — the evidence-audit `model.Dossier` and the `briefing_model.OperativeBriefingBundle` — into shippable artifacts (Markdown, PDF) and does **no clinical reasoning of its own**. It runs **two parallel renderer stacks**:
...
- **Figure dual-lane:** retrieved plates (P1 `figure_retriever`/`figure_guards`) vs. generated schematics (P4 `figures_gen`), both surfacing into the rendered dossier/briefing and the web figure grids; `figures_gen/guard.py` reuses `neuro_core.figure_guards.figure_offtarget`.
...
- **(Removed) Streamlit** (`app/streamlit_app.py`): was the *only* surface with a password gate (`APP_PASSWORD`); reused the engine + P4 `board_view`. Deleted in `b561e45` — see the Auth nuance note in §8.
...
- **Auth nuance (from P5).** The FastAPI surface is intentionally **auth-free** by design (`api/server.py`'s own docstring: "the loop spec forbids any APP_PASSCODE/login gate on this surface"). Streamlit was the only surface that ever had a password gate (`APP_PASSWORD`); with its removal, no surface has one. Phone exposure relies on network reachability, not a password.
- **Two build entry points.** CLI uses `pipeline.generate`/`generate_case`; the API uses `build_dossier`/`build_case_dossier`/`build_briefing_bundle` — sibling wrappers (P2-confirmed).
