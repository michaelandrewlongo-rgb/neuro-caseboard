...
### Task 4: Dossier surface — split into 4a (body) + 4b (shell+telemetry)
...
**Why split:** the full surface (4 rich .tsx files) overflowed one subagent's output budget; doing it in two ~2-file passes keeps each dispatch reliable.

- [x] **Task 4a — Dossier body (`DossierView.tsx` + `EvidenceBar.tsx`).** Rebuild the dossier body `1fr/372px`: section cards (gradient A/B letter badge, intro, claims) + right rail (Figures, Corpus sources, Contemporary literature plum). Claim card: left status bar + glow dot, status eyebrow (SUPPORTED sage / TO VERIFY ochre / QUARANTINED brick), claim text, accent-colored "Why:" label, checkbox sub-items (`accent-color:#3f9690`), teal `[n]` citation chips + "→ Fig n"; quarantined claims show "Held back:" note + no sources. Restyle `EvidenceBar.tsx` to the palette. Add a `filter?: 'all'|'supported'|'verify'|'quarantine'` prop to `DossierView` (default `'all'`) that dims non-matching claims to `opacity:.22` over `.3s` — the segmented control that sets it is wired in 4b. Map onto real `DossierSection`/`DossierClaim`/`EvidenceSummary` fields; preserve honest-degradation. Do NOT edit Build.tsx or create PlanningMetrics here.
- [x] **Task 4b — Dossier shell + telemetry (`Build.tsx` + new `PlanningMetrics.tsx`).** In `Build.tsx`: case header (teal eyebrow `ACTIVE CASEBOARD · PRE-OP DOSSIER · v3`, H1 case title, mono attribute chips, "Export PDF" ghost + "Rehearse" crimson buttons wired to EXISTING handlers); telemetry grid `1.15fr/1fr/1.05fr`: **Risk topology** `RiskRadar` (7 axes CN VII / CN VIII / Brainstem / AICA / CSF / Lower CN / Cerebellum, predicted-crimson + post-mitigation-teal-dashed polygons), **Evidence integrity** `EvidenceGauge` (3 rings Supported-sage / Verify-ochre / Quarantine-brick, center count, legend), **Planning metrics** = new `PlanningMetrics.tsx` (labeled bars Facial-pres sage / Hearing-pres ochre / GTR teal + stat tiles EST OR TIME, CSF LEAK RISK brick); evidence filter segmented control ALL/SUPPORTED/VERIFY/QUARANTINE (counts, active teal) whose state is passed as the `filter` prop into `<DossierView>` (from 4a). **Honest data mapping (clinical tool, binding):** drive `EvidenceGauge` from the real `EvidenceSummary`; if `BuildResponse` has no backing data for `RiskRadar` axes or `PlanningMetrics` percentages, render those in an honest "not available" state — DO NOT fabricate clinical risk/percentage numbers. Preserve all existing data flows + loading/error/unavailable states; do not modify api.ts.
...
**Why split:** 8 files exceeds one subagent's safe output budget (see Task 4). 6a = page shell + telemetry charts; 6b = answer body + right-rail components.

- [x] **Task 6a — Ask shell + telemetry (`Ask.tsx` + new `CitationAudit.tsx`, `StructuresRadar.tsx`).** In `Ask.tsx`: hero (eyebrow `ASK · CITED ANSWER ENGINE`, H1 "Ask the corpus", search field + crimson "Ask" button, example chips); status line ("N of M claims grounded…" from the REAL response); telemetry row `1fr/1fr` rendering new `CitationAudit` + `StructuresRadar`. Create `CitationAudit.tsx` (wraps `charts/EvidenceGauge` concentric, center "g/t · GROUNDED", sage/ochre legend, fed by the answer's grounded/verify counts) and `StructuresRadar.tsx` (wraps `charts/RiskRadar` crimson, axes = anatomy named in the answer). HONEST DATA: drive CitationAudit from real grounded/verify counts; if the answer has no extractable anatomy for StructuresRadar, render it honest-empty — do not fabricate. Keep the existing `<AnswerView>`/right-rail render in place (6b restyles them). Preserve clarification/unavailable/error states + abortable request. Do not edit AnswerView/citations/FigureGrid/SourcesList/LiteratureBlock here.
- [x] **Task 6b — Ask answer body + right rail (`AnswerView.tsx`, `citations.tsx`, `FigureGrid.tsx`, `SourcesList.tsx`, `LiteratureBlock.tsx`).** Restyle `AnswerView`/`react-markdown` body to 15px/1.72 `#ece7e1`; render inline `[n]` citations as teal pills (bg `rgba(63,150,144,.12)`, text `#6fc0b8`) via `citations.tsx`; ochre "VERIFY" callout for flagged claims. Right rail: `FigureGrid` (dashed FIG placeholders → real `/api/figure` when `image_available`), Contemporary-literature lane in `LiteratureBlock` (plum `[L#]`), `SourcesList` (teal `[n]` + book/chapter/page). Map onto real response fields; preserve honest-degradation. Do not edit Ask.tsx/the new chart-consumers (6a done).
