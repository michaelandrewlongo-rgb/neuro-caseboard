...
def test_missing_file_returns_empty_set(tmp_path):
    assert load_suppressions(tmp_path / "nope.json") == set()


def test_active_suppression_is_loaded(tmp_path):
    p = tmp_path / "s.json"
    p.write_text(json.dumps([{"fingerprint": "abc123", "reason": "known noise"}]),
                 encoding="utf-8")
...
def test_expired_suppression_is_dropped(tmp_path):
    p = tmp_path / "s.json"
    p.write_text(json.dumps([{"fingerprint": "abc123", "expires": "2020-01-01"}]),
                 encoding="utf-8")
