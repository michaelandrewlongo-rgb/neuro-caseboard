...
def resolve_textbook_enabled(env: Mapping[str, str] | None = None) -> bool:
    values = env if env is not None else os.environ
    raw_value = values.get("CASEPREP_TEXTBOOK", "0").strip()
    if raw_value == "0" or raw_value == "":
        return False
    if raw_value == "1":
        return True
    raise CasePrepConfigurationError(
        "CASEPREP_TEXTBOOK must be 0 or 1",
        details={"field": "CASEPREP_TEXTBOOK", "value": raw_value},
    )


__all__ = ["resolve_retrievers_v2_enabled", "resolve_textbook_enabled"]
